...
let test_bike tkn n =
  let bike_rq =
    let uri =
      "https://api.cyclocity.fr/contracts/ljubljana/bikes?number=" ^ string_of_int n
      |> Uri.of_string
    in
    let headers =
      Header.of_list
    ...
    body |> Cohttp_lwt.Body.to_string
  in
  Lwt_main.run bike_rq

let _test_station tkn n =
  let station_rq =
    let uri =
    ...
    body |> Cohttp_lwt.Body.to_string
  in
  Lwt_main.run station_rq
  
  

let () =
  let access_token = get_access_token in
  let stations = get_stations access_token in
  let bikes = get_bikes access_token in
  let _iter_fun stnum =
    let station_bikes = Hashtbl.find_all bikes stnum in
    let station_name = List.assoc stnum stations in
    print_string (station_name ^ ": ");
    List.iter
  ...
  print_endline (test_bike access_token 3430);
  print_endline (test_bike access_token 606)
